...
test('RLS de comissões usa beneficiary_id, pois a tabela não possui user_id',()=>{const policies=sql.split('alter table inventory enable row level security')[1];assert.match(policies,/commission_ledger[\s\S]*beneficiary_id\s*=\s*auth\.uid\(\)/i);assert.doesNotMatch(policies,/create policy[^;]*commission_ledger[^;]*user_id\s*=\s*auth\.uid\(\)/i)})
test('policies de proprietário são explícitas, sem loop dinâmico perigoso',()=>{const policies=sql.split('alter table inventory enable row level security')[1];assert.doesNotMatch(policies,/foreach[\s\S]*create policy/i);assert.match(policies,/create policy own_or_admin on public\.inventory/i)})
